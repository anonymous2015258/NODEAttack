...
class Dopri5Solver(AdaptiveStepsizeODESolver):
    ...
    def advance(self, next_t):
        ...
        #print("in advance")
        iiop=torch.tensor(0.0, device="cuda")
        n_steps = 0
        sm=torch.tensor(0.0, device="cuda")


        while next_t > self.rk_state.t1:
            assert n_steps < self.max_num_steps, 'max_num_steps exceeded ({}>={})'.format(n_steps, self.max_num_steps)
            self.rk_state,mse = self._adaptive_dopri5_step(self.rk_state)
            n_steps += 1
            '''if(n_steps == 1):
                iiop=self.rk_state.dt'''
            #op.append(self.rk_state.dt)

            if (n_steps == 1):
                sm = self.rk_state.dt
            '''else:
                sm=torch.add(sm,self.rk_state.dt)'''

            #iiop=self.rk_state.t1
            #print("next_t ", next_t)
            #("self.rk_state.t1 ", self.rk_state.t1)
        ...
        f.close()'''
        #torch.div(sm,n_steps),
        return _interp_evaluate(self.rk_state.interp_coeff, self.rk_state.t0, self.rk_state.t1, next_t), sm,n_steps

    def _adaptive_dopri5_step(self, rk_state):
        #print("_adaptive_dopri5_step")
        ...
        return rk_state, mean_sq_error_ratio
